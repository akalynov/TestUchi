...
let firstNum = randomInRange(6, 9);
let secondNum = randomInRange(11, 14) - firstNum;
...
expression.innerHTML = `
	<span class="first-number">${firstNum}</span> + <span class="second-number">${secondNum}</span> <span class="equally-span"> = </span> <span class="amount">?</span>
`;

let expressionFirstNum = expression.querySelector('.first-number');
let expressionSecondNum = expression.querySelector('.second-number');
...
let endFirstArc = 34 + (cm * firstNum); // Координаты конца первой дуги
...
let endSecondArc = 34 + (cm * secondNum) + (cm * firstNum); // Координаты конца второй дуги
...
(function createFirstArc() { //Рисуем первую дугу
    ...
    ctx.lineWidth = 3;
    ctx.strokeStyle = 'red';
    ctx.moveTo(34, 85); //левый нижний угол canvas
...
firstNumberInput.oninput = () => checkValue(firstNumberInput, firstNum, expressionFirstNum);
secondNumberInput.oninput = () => checkValue(secondNumberInput, secondNum, expressionSecondNum);
